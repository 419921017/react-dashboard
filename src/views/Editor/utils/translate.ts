 ...
 */
export function calculateRotatedPointCoordinate(point: ICoordinate, center: ICoordinate, rotate: number) {
  /**
   * 旋转公式：
   *  点a(x, y)
   *  旋转中心c(x, y)
 ...
 */
export function getRotatedPointCoordinate(style: ICoordinateStyle, center: ICoordinate, name: string) {
...
// 求两点之间的中点坐标
export function getCenterPoint(p1: ICoordinate, p2: ICoordinate) {
  return {
    x: p1.x + (p2.x - p1.x) / 2,
    y: p1.y + (p2.y - p1.y) / 2,
  }
